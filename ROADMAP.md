...
**过时 PLAN 标记**:`PLAN-fix-cwd-ttl-vs-resume.md` 描述的 bug(TTL 清理误删 resume cwd)**已修复**——`server-agent.ts:334` 的 `cleanupExpiredRunCwds` 已用 `listProtectedSdkCwds()` 跳过被 chat_sessions 引用的 cwd。该 PLAN 可删。
