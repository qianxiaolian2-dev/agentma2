...
export function genMinecraftServerCode(server: string, port: string) {
  return [
    `// MCP Server: ${server} — Minecraft Bot`,
    ...
    `  username: process.env.MC_USERNAME || 'LianLian',`,
    `  auth: 'offline',`,
    `});`,
    ``,
    ...
    `bot.on('end', () => { console.log('[bot] disconnected'); setTimeout(() => process.exit(1), 5000); });`,
    ``,
