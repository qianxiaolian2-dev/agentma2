        ...
        <div className="grid-2">
          {filtered.map(tool => (
            <div key={tool.name} className="tool-card" onClick={() => setSelectedTool(tool)} style={{ cursor: 'pointer' }}>
              <div className="flex-between"><div className="tool-card-name">{tool.name}</div>{'source' in tool && <span className="badge badge-info">自定义</span>}</div>
              ...
              {'source' in tool && <button className="btn btn-sm btn-danger mt-2" onClick={e => { e.stopPropagation(); handleDelete(tool.name); }}>删除</button>}
            </div>
          ))}
        </div>
      )}
