...
async function handleStream(response, token) {
  ...
  async function handleEvent(data) {
    ...
    if (data.type === 'delta') {
      ...
      finalText += text;
      if (text.includes('Agent(') || text.includes('Task(')) sawAgentToolUse = true;
      return;
    }
    ...
    if (data.type === 'task_started') {
      sawTaskStarted = true;
      observedSubagentType = data.subagentType || observedSubagentType;
      console.log(`task_started ${data.taskId} ${data.subagentType || data.taskType || '-'}`);
      return;
    }
    if (data.type === 'task_progress') {
      sawTaskProgress = true;
      observedSubagentType = data.subagentType || observedSubagentType;
      console.log(`task_progress ${data.taskId} ${data.subagentType || '-'} ${data.summary || data.lastToolName || ''}`);
      return;
    }
    if (data.type === 'task_notification') {
      sawTaskNotification = true;
      console.log(`task_notification ${data.taskId} ${data.status} ${data.summary || ''}`);
      return;
    }
    if (data.type === 'error') {
...
async function main() {
  ...
  try {
    ...
    const subagents = {
      echoer: {
        description: 'Use this subagent whenever the user asks to delegate a marker relay task.',
        prompt: `You are the echoer subagent. Return only the exact marker requested by the parent agent. No extra words.`,
        tools: [],
        model,
        maxTurns: 1,
      },
    ...
    const template = {
      id: `agent-subagent-smoke-${stamp}`,
      name: 'Subagent Smoke',
      description: 'Smoke test template with a real SDK subagent.',
      systemPrompt: 'You are running a smoke test. You must use the Agent tool exactly once when asked to delegate. Do not answer the delegated marker directly.',
      model,
      tools: ['Agent'],
      subagents,
      mcpServers: [],
    ...
    const savedTemplates = await requireOk('save agent template', fetchJson(`${baseUrl}/api/agents`, {
      method: 'PUT',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify([template]),
    }));
    const savedTemplate = Array.isArray(savedTemplates) ? savedTemplates.find((item) => item.id === template.id) : null;
    ...
    const chat = await fetch(`${baseUrl}/api/chat`, {
      ...
      body: JSON.stringify({
        messages: [{
          role: 'user',
          content: `Delegate this to the echoer subagent with the Agent tool. Ask it to return exactly ${marker}. After the subagent finishes, reply with exactly ${marker}.`,
        }],
    ...
    const checks = {
      templateSaved: Boolean(savedTemplate?.subagents?.echoer) && savedTemplate?.tools?.includes('Agent'),
      resultSuccess: stream.resultSubtype === 'success',
      sawAgentToolUse: stream.sawAgentToolUse,
      sawTaskStarted: stream.sawTaskStarted,
      sawTaskEvent: stream.sawTaskProgress || stream.sawTaskNotification,
    ...
    console.log(`observedSubagentType ${stream.observedSubagentType || '-'}`);
    ...
    if (failed.length) {
      throw new Error(`subagent smoke failed: ${failed.join(', ')}`);
    }
  } finally {
